...
`nest build` is a wrapper on top of the standard `tsc` compiler (for [standard projects](/cli/overview#project-structure)) or the webpack compiler (for [monorepos](/cli/overview#project-structure)). It does not add any other compilation features or steps except for handling `tsconfig-paths` out of the box. The reason it exists is that most developers, especially when starting out with Nest, do not need to adjust compiler options (e.g., `tsconfig.json` file) which can sometimes be tricky.
