...
[nestia](https://github.com/samchon/nestia) is an evolved SDK and Swagger generator.

It analyzes your NestJS server code in the compilation level. With `nestia` and compilation level analyzer, you don't need to write any swagger or `class-validator` decorators. All you need to do is use the `nestia` CLI as shown below.
...
export async function test_shopping_sale_question_store(
    connection: api.IConnection, 
    sale: IShoppingSale
): Promise<ISaleQuestion> {
    // WRITE A QUESTION
    const question: ISaleQuestion = 
        await api.functional.shoppings.sales.questions.store(
            connection,
            "general",
            sale.id,
            {
                title: "How to use this product?",
                body: "The description is not fully enough.",
                files: []
            }
        );
...
Also, `nestia` can understand descriptive comments. 

Those descriptive comments would be resurrected in SDK or Swagger.
...
Also, descriptive comments are fully revived in SDK, regardless of where they are written.
...
function store(connection, sectionCode, saleId, articleId, body) {
    return nestia_fetcher_1.Fetcher.fetch(connection, store.ENCRYPTED, store.METHOD, store.path(sectionCode, saleId, articleId), body);
}
exports.store = store;
(function (store) {
    store.METHOD = "POST";
    store.PATH = "/consumers/:section/sales/:saleId/articles/:articleId/comments";
    store.ENCRYPTED = {
        request: false,
        response: false,
    };
    function path(sectionCode, saleId, articleId) {
        return `/consumers/${sectionCode}/sales/${saleId}/articles/${articleId}/comments`;
    }
    store.path = path;
