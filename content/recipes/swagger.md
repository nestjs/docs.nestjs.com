...
> warning **Hint** There's an `@ApiModelPropertyOptional()` shortcut decorator which helps to avoid continuous typing `{{"@ApiModelProperty({ required: false })"}}`.  There is also `@ApiResponseModelProperty()` shortcut decorator for response classes which only accepts `type` and `example` metadata parameters.
