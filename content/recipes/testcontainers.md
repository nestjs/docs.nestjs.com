...
> info **Note** This example uses PostgreSQL as a simple starting point for working with Testcontainers in NestJS. It is not meant to cover every possible integration or dependency setup. For more advanced configurations, please refer to the Testcontainers documentation and module guides.
...
In our example above, the `PostgreSqlContainer` class from Testcontainers requires a Docker image as a parameter, allowing you to specify the version or tag of the service required (e.g., `postgres:18.3-alpine`). To avoid potential timeouts during tests, especially with slower network connections, consider downloading/pulling the image beforehand or increasing the Jest test timeout. Note that this only affects the initial test run; once the image is downloaded, subsequent tests will run without delay.By default, the `PostgreSqlContainer` above will create a **database** named `test`, and the credentials **username** `test` and **password** `test`, If you want to configure it to be different, we can these methods `withDatabase`, `withUsername` and `withPassword` to modify the default values.
