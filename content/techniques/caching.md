...
Note that the Redis service does not automatically shut down; you will need to enable shutdown hooks in `main.ts`, and add a hook to `app.module.ts`.
...
Every module that uses Redis for cache must include the CacheModule option bundle.
