...
As you can see, the definition is empty although the class has a few declared properties.
In order to make the class properties visible to the `SwaggerModule`, we have to either annotate them with the `@ApiProperty()` decorator (or `@ApiOptionalProperty()` for optional fields) or use the CLI plugin (read more in the Plugin section) which will do it automatically:
