...
| `timeout` | Maximum time (in milliseconds) to wait for the shutdown sequence before exiting anyway. The process state is not trustworthy after a fatal error, so this keeps a stuck hook from hanging the process forever. | `5000` |
